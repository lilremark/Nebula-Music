<div align="center">
  ...
  **A polished, self-hosted web player for your Subsonic music library.**

  Stream from Navidrome, Gonic, Airsonic, and other Subsonic-compatible servers
  through a responsive interface built for desktop and mobile.

  [![Version](https://img.shields.io/badge/version-2.1.2-0ea5e9?style=flat-square)](https://github.com/lilremark/Nebula-Music)
...
- Automatic transcoding rules for formats that are unreliable in browsers, including ALAC and M4A
- Internet radio playback for direct streams and HLS playlists

### Listening Experience

- Seven Web Audio visualizers: Bars, Wave, Circle, Mirror, Spectrum, Particles, and Hexagon
- Expandable full-screen player, desktop sidebar player, floating mini-player, and mobile player bar
- Lyrics and queue views within the full-screen player
...
- Persistent sorting and filtering by genre, year, and other library metadata
...
- Browser-stored authentication using a Subsonic token and salt instead of the raw password

## Compatibility

Nebula Music communicates with servers that implement the
[Subsonic API](http://www.subsonic.org/pages/api.jsp), including:
...
Your music server must be reachable from the browser running Nebula. HTTPS and
...
Open [http://localhost:3000](http://localhost:3000), then enter your server URL,
username, and password. No environment variables are required.
...
The included multi-stage image builds Nebula with Node.js and serves the static
...
flowchart LR
    ...
    API --> Server["Subsonic-compatible server"]
...
directory to Netlify, Cloudflare Pages, Amazon S3, or any other static host.

Because Nebula connects to the music server directly from the browser, the
deployed origin must be permitted by your server's CORS policy.
...
first so the proposed behavior can be discussed before implementation.

1. Fork the repository.
2. Create a branch: `git checkout -b feature/my-change`.
3. Install dependencies: `npm install`.
4. Make and verify your changes: `npm run build`.
